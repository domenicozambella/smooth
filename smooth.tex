...
The elements of $S_n$ are called \emph{finite samples\/} (we will mainly use $\NN$-valued samples and interpret these as finite multisets). Let \emph{$\bar\U$\/} be the multi-sorted structure $\<\U,\RR,(S_n)_{n\in\omega}\>$.
...
\begin{proof}
  ...
  Extend $f$ to an automorphism of $\bar\U$ by requiring that $f$ is the identity on $\RR$ and $s\,b=(f\,s)(f\,b)$.
  Then $\bar\U\models\phi(f\,s,a,r,t)$, so $\bar M\models\phi(f\,s,a,r,t)$ follows by induction hypothesis.
\end{proof}
...
\begin{proof}
  Let $u$ be a variable of same sort as $\nsS_{|x|}$.
  We claim that the type $p(u)$ defined below is finitely consistent 

  \ceq{\hfill p(u)}{=}{\Big\{\sum_{\phi(x)}ux=\mu\phi(x)\quad:\ \ \phi(x)\in\Delta\Big\}}

  Let $\{\phi_1(x),\dots,\phi_n(x)\}\subseteq\Delta$.
  It suffices to show that there is $s\in\nsS_{|x|}$ such that
  ...
  Without loss of generality we can assume that $\{\phi_1(x),\dots,\phi_n(x)\}$ is a Boolean algebra with atoms $\phi_1(x),\dots,\phi_k(x)$ for some $k\le n$.
  Pick some $a_1,\dots,a_k\in\U^{|x|}$ such that $a_i\models\phi_i(x)$.
  Pick $s\in\nsS_{|x|}$ with support $\{a_1,\dots,a_k\}$ and such that
...
In other words, if $\bar\tp(s/\bar\U)$ is finitely satisfied in $\bar M$. 
We prefer not to mention finite satisfiability in this context to avoid clash with the terminology in the next section.
Note also that the condition above is equivalent to
...
We present a stronger version of pseudofiniteness.
This is obtained by strengthening \ssf1 as follows. 
Require that for every $\phi(u)\in\bar L(\bar M)$  

\ceq{\ssf3.\hfill \phi(s)}{\IMP}{\phi(s\cdot \Indicator_A)} for some finite $A\subseteq M$ 

where $\Indicator_M$ is the indicator of $A$.

\section{Invariant samples}

Here we introduce the notions of invariant and finitely satisfiable samples.
There are two sensible variants.
We consider the most stringent variant, which is also the easier to deal with.

We write $\Aut(\ns\U/A,\{\U\})$ for the set of automorphisms of $\ns\U$ that fix $A$ pointwise and $\U$ setwise. 
Every automorphism $f\in\Aut(\ns\U)$ has a canonical extension to an automorphism in $\Aut(\bar\U)$, which we denote by the same symbol $f$.
Namely, this extension is the identity on $\ns\RR$ and maps $s\in\nsS_n$ to the unique $fs\in\nsS_n$ such that $(fs)\,(fa)=s\,a$.

Given $s\in\nsS_{|x|}$, we say that $\mu_s$ is $A$-invariant if for every $f\in\Aut(\U/A,\{\U\})$
...
\begin{definition}\label{def_fin_sat}
  We say that $f$ is \emph{finitely satisfiable\/} in $M$ if for every $\phi(x)\in L(\U)$

  \ceq{\hfill \phi(\supp f)\neq\0}{\IMP}{\phi(M)\neq\0.}\QED
\end{definition}

The following lemma shows that the finite satisfiability of a sample corresponds to the finite satisfiability of the associated Keisler measure.
...
\begin{proof}
  ...
  Apply the same reasoning as in the proof of Lemma~\ref{lem_existence_sample} and note that Definition~\ref{def_windependence} is automatically saisfied.
\end{proof}
...
We say that $s\in\nsS_{|x|}$ is smooth over $\Delta$, or over $A$ when $\Delta=L(A)$, if for every $\phi(x)\in L(\U)$ and every $\epsilon\in\RR^+$ there are $\psi_i(x)\in\Delta$ such that $\psi_1(x)\imp\phi(x)\imp\psi_2(x)$ and 


\ceq{\hfill\mu_s\psi_1(x)}{\approx_\epsilon}{\mu_s\psi_2(x)}

An easy argument of compactness yields this uniform version fo the condition above.

\begin{fact}
  The following are equivalent
  \begin{itemize}
    \item[1.] $s\in\nsS_{|x|}$ is smooth;
    \item[2.] for every $t\in\nsS_{|x|}$, if $\mu_s\phi(x)\approx\mu_t\phi(x)$ for all $\phi(x)\in L(M)$ then the same holds for all $\phi(x)\in L(\U)$; 
    \item[3.] for every $\phi(x,z)\in L$ and every $\epsilon\in\RR^+$ there is a  
  \end{itemize}
\end{fact}
...
\begin{definition}\label{def_windependence}
  We say that $s$ is \emph{weakly independent\/} of $\Delta$ over $M$ if 

  \ceq{\hfill \phi(a)}{\IMP}{\phi(M)\neq\0} 
  
  for every $\phi(x)\in\Delta$ and  $a\in\supp s$.\QED
\end{definition}
