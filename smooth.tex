...
\renewcommand*{\emph}[1]{%
   \smash{\tikz[baseline]\node[rectangle, fill=green!25, rounded corners, inner xsep=0.5ex, inner ysep=0.2ex, anchor=base, minimum height = 2.7ex]{#1};}}
...
In this section we introduce hypefinite samples and prove in Lemma~\ref{lem_existence_sample} that all global Keisler measures are generated by some hyperfinite sample.
...
These will interpreted as \textit{signed measures concentrated on a finite set\/} (which would be more appropriate but a much too long name).
...
Clearly, $|\bar\U|=\kappa$.
We call the first sort the \emph{home\/} sort; the second the \emph{real\/} sort; the others are collectively named \emph{sample sorts.}
...
\begin{lemma}\label{lem_existence_sample}
  ...
  \ceq{\#\hfill\mu_s\phi(x)}{=}{\mu\phi(x)}\hfill for every $\phi(x)\in L(\U)$.
\end{lemma}
...
If $M\preceq\U$ we define \emph{$\bar M$\/} to be the structure $\<M,\RR,(S_n{\restriction} M)_{n\in\omega}\>$, where \emph{$S_n{\restriction} M$\/} is the set of samples $s\in S_n$ such that $\supp s\subseteq M^n$.
...
\begin{proof}
  ...
  Rewriting formulas in this only introduces new quantifiers over the reals.
  ...
  The only interesting case concern the existential quantifiers of samle sort, say $\E u$ where $u$ has the sort of $S_{|x|}$.
...
The following example shows that the fact above fails if we do not assume $M$ to be sufficiently saturated.

\begin{example}
  Let $\U=\ns\NN$.
  Then $\bar\U=\<\ns\NN,\RR,(S_n)_{n\in\omega}\>$.
  Let $\bar M=\<\NN,\RR,(S_n{\restriction}\NN)_{n\in\omega}\>$. 
  We can write a sentence $\phi$ that says that there are samples whose supports is arbitrary large. 
  Namely, let $x,y$ de variables of the home sort and let $u$ have sort $S_1$.
  Then $\phi$ is the sentence  $\A x\; \E u\; \A y<x\; (u\,y\neq0)$.
  Clearly $\bar M\models\phi$ while $\bar\U\notmodels\phi$.\QED
\end{example}
...
\begin{conjecture}[(of fundamental importance, I guess)]\label{conj_main}
   For every bounded $s,t\in\ns S_{|x|}$ the following are equivalent
  \begin{itemize}
    \item[1.] $s\equiv_Mt$\hfill clearly, equivalence is w.r.t.\@ the language $\bar L$;
    \item[2.] $\mu_s{\restriction} M\approx\mu_t{\restriction} M$.
  \end{itemize}
\end{conjecture}

Write $\bar L^{\!\cdot}$ for the smallsest set of formulas containing $L$, $\bar L_{\rm qf}$ and closed under quantification in the sort of the reals.
Write $\dot\equiv_M$ for elementary equivalence restricted to formulas in $\bar L^{\!\cdot}$.

\begin{conjecture}[(weaker but true, I guess)]\label{conj_main}
  For every bounded $s,t\in\ns S_{|x|}$ the following are equivalent
 \begin{itemize}
   \item[1.] $s\mathrel{\dot\equiv}_Mt$
   \item[2.] $\mu_s{\restriction} M\approx\mu_t{\restriction} M$.
 \end{itemize}
\end{conjecture}
...
It translates almost literally to samples.
...
The following notion will be proven redundant in Fact~\ref{fact_smoothness} but we introduced it for emphasis.
...
Precisely, $s$ is uniformly smooth if for every $\phi(x,z)\in L$ and every $\epsilon\in\RR^+$ there are some finitely many formulas $\psi_1(x),\dots,\psi_n(x)\in L(M)$ such that for every $b\in\U^{|z|}$ there is an $i\in(n]$ such that

\ceq{\hfill\psi_i(x)}{\imp}{\phi(x,b)}

\ceq{\hfill\mu_s\psi_i(x)}{\approx_\epsilon}{\mu_s\phi(x,b).}

The following is an important consequence of uniformity.

\begin{fact}
  Let $s\in\nsS_{|x|}$ be a non-negative bounded sample
  that is uniformly smooth over $M$. Then for every $\epsilon\in\RR^+$ there is a $t\in\bar M$ such that $\mu_t\approx_\epsilon\mu_s$.
\end{fact}

\begin{proof}
  For any given $\epsilon\in\RR^+$ choose $t\in\bar M$ such that $\mu_t$ coincides with $\st\big(\mu_s\big)$ on the Boolean algebra generated by $\psi_1(x),\dots,\psi_n(x)$.
\end{proof}


\begin{fact}\label{fact_smoothness}
  Let $s\in\nsS_{|x|}$ be a non-negative bounded sample. 
...
\begin{proof}
  ...
  \ssf1$\IMP$\ssf3\quad Fix $\epsilon\in\RR$ and $\phi(x)\in L(\U)$.
  We claim that $\mu_s\phi(x)\approx_\epsilon\mu_t\phi(x)$.
  Assume that $s$ is smooth and let $\psi(x)\in L(M)$ be as in the definition of smooth but with $\epsilon/2$ for $\epsilon$.
  Then $\mu_t\psi(x)\approx\mu_s\psi(x)\approx_{\epsilon/2}\mu_s\phi(x)$.
  Then $\mu_s\phi(x)-\mu_t\phi(x)\le\mu_s\phi(x)-\mu_t\psi(x)\le\epsilon$.
  The same argument applied to $\neg\phi(x)$ yields $\mu_t\phi(x)-\mu_s\phi(x)\le\epsilon$.
  This proves the claim and \ssf3 follows.

  \ssf3$\IMP$\ssf1\quad (Da ripulire.) Suppose $\phi(x,b)$ witness the failure of \ssf1. That is, for some $\epsilon\in\RR^+$, we have $\mu_s\psi(x)\napprox_\epsilon\mu_s\phi(x,b)$ all $\psi(x)\in L(M)$.
  Compactness and the fact that $\bar\U\preceq\ns\bar\U$ ensure the existence of $t$ of maximal (hyperfinite) cardinality such that $t\equiv_Ms$ and
  ...
  We claim that there is a formula  $\psi(x)\in L(M)$ such that $\mu_t\phi(x,b)-\mu_t\psi(x)<\epsilon/2$.
  In fact, this is true in $\bar\U$ and therefore in $\ns\bar\U$.
  From this we obtain

  \ceq{\hfill\epsilon/2}{\le}{\mu_s\phi(x,b)-\mu_s\psi(x)+\mu_t\psi(x)-\mu_t\phi(x,b)}

  which contraddicts \ssf3. 
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Analytic samples}

A stronger notion of smoothness is natural in our context.
Apparently, it has no parallel for Keisler measures. 
Tentatively, we say that a sample $s\in\nsS_{|x|}$ is \emph{analytic\/} (just a hyperbole of \textit{smooth\/}) if $t\equiv_M s$ implies $t\equiv_\U s$ for every $t\in\nsS_{|x|}$.
  
I do not know if this notion has non trivial examples. 
On the other hand, if Conjecture~\ref{conj_main} is true, it may be equivalent to smoothness.
...
There are a few distinct notions of finite satisfiabilty that apply to our context.
Pseudo\-finiteness it is the notion that best fit with that of analytic samples.
(A weaker notion of finite satisfiability that almost literally translate that of Keisler measures will be introduced later.)

We say that a sample $s\in\nsS_{|x|}$ is \emph{elementary\/} over $M$ if
...
By Fact~\ref{fact_elementarity}, every sample is elementary over any $\omega$-saturated model.
If \ssf1 holds for every $\phi(u)\in\bar L(\U)$ then we say that $s$ is \emph{pseudofinite.}
By the standard argument of existence of global coheirs, for every elementary $s\in\nsS_{|x|}$ there is a pseudofinite $s'\in\nsS_{|x|}$ such that $s'\equiv_{M}s$.
...
\begin{proof}
  If $\phi(M)\subseteq\supp s$ then 
  
  \ceq{\hfill s }{\models } {\A x \Big[\phi(x)\imp x\in\supp u\Big]}

  By pseudofiniteness this formula is satisfied also by some $t\in\bar M$, hence $\phi(M)$ is finite.
\end{proof}

\begin{fact}
  If $s$ is elementary and analytic then it is pseudofinite.
\end{fact}

\begin{proof}
  Let $p(u)=\tp(s/M)$. 
  As $s$ is elementary, $p(u)$ is finitely satisfied in $M$.
  Then it hase an extension to a complete type over $\U$ that is finitely satisfied in $M$.
  By analyticity, $s\models p(u)$.
\end{proof}
% \begin{fact}
...
Again, the following notion does not literally translare the homonymous notion for Keisler measures; it is a stronger notion introduced in analogy to those in the two paragraphs above.

We say that the sample $s\in\nsS_{|x|}$ is \emph{definable\/} over $M$ if for every $\phi(u,x)\in\bar L(\U)$ there is a type $p(x)\subseteq\bar L(M)$ such that $p(\U)=\phi(s,\U)$.

\begin{fact}
  If $s\in\nsS_{|x|}$ is analytic over $M$ then it is definable over $M$.
\end{fact}

\begin{proof}
  Let $q(u)=\tp(s/M)$ and define $p(x)=\E u\,\big[q(u)\wedge\phi(u,x)\big]$. By analyticity $p(\U)=\phi(s,\U)$.
\end{proof}


\hrulefill

Qui sotto solo scemenze.
Magari anche sopra.
...
A sample $s\in\nsS_{|x|}$ is called \emph{generically stable\/} over $M$ if it is both pseudofinite and definable over $M$.
Again, this notion is stronger than the homonymous notion for Keisler measures.

\begin{fact}[(T is NIP?)]
  Let $s\in\nsS_{|x|}$ be non negative and generically stable.
  Let $\phi_i(u,x)\in\bar L(M)$, for $i=1,\dots,n$, be such that $\phi(s,\U)$ covers $\U^{|x|}$.
  Then there is a $t\in\bar M$ such that $\phi_i(t,\U)=\phi_i(s,\U)$ for all $i$.
\end{fact}

\begin{proof}
  Let $p_i(x)\subseteq\bar L(M)$ be such that $p_i(\U)=\phi_i(s,\U)$.
  by compactness there are some formulas $\psi_i(x)\in p_i(x)$  such that $\psi_i(\U)=\phi_i(s,\U)$.
\end{proof}

\begin{fact}[(T is NIP?)]
  Let $s\in\nsS_{|x|}$ be non negative, bounded, and generically stable.
...
\begin{proof}
  Fix $n$ and define $\epsilon=1/(n+1)$ and $\alpha_i=i/(n+1)$ for $i=1,\dots,n$.
  Let $p_i(x)\subseteq\bar L(M)$ be such that $p_i(\U)=\{b:\mu_s\phi(x,b)\approx_\epsilon\alpha_i\}$.
  As the sets $p_i(\U)$ cover $\U$, by compactness there are some formulas $\psi_i(x)\in\bar L(M)$ such that $\psi_i(\U)=\{b:\mu_s\phi(x,b)\approx_\epsilon\alpha_i\}$.
  By pseudofiniteness there is a $t\in\bar M$ such that $\mu_t\phi(x,b)\approx_\epsilon\mu_s\phi(x,b)$ for all $b$.
\end{proof}
...
\begin{lemma}
  Let $\mu$ be as in Lemma ??? with $\Delta=L(\U)$.
  Assume that
