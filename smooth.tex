...
\section{Bootstrapping hyperfinite samples}

In this section we introduce hypefinite samples and prove in Lemma~\ref{lem_existence_sample} that all Keisler measures are generated by some hyperfinite sample.
...
\begin{example}
  The following example shows that the fact above cannot be improved.
  Let $\U=\ns\RR$.
  Then $\bar\U=\<\ns\RR,\RR,(S_n)_{n\in\omega}\>$.
  Recall that $S_1=\{s:\ns\RR\to\RR\}$.
  Let $x$ and $u$ be variables of sort $\ns\RR$ and $S_1$, respectively.
  The sentence 
  
  \ceq{\hfill\E x\, \A u\,\Big[ x\in\supp u}{\imp}{0<|x|<|ux|\Big]}
  
  
  holds in $\bar\U$.
  But it does not hold in $\bar\RR$.\QED
\end{example}
...
The following notion will be proven redundant (Fact~\ref{fact_smoothness}) and it is only introduced for emphasis.
...
\section{Analytic samples}
  ...
  A stronger notion of smoothness is natural in our context which has no parallel for Keisler measures. 
  ...
  (I am not sure this notion has non trivial examples.)
...
Pseudofiniteness it is the notion that best fit with that of analytic samples.
...
Again he following notion does not literally translare the homonymous notion for Keisler measures, but is stronger in analogy to the two paragraphs above.
...
\begin{proof}
  Let $q(u)=\tp(s/M)$ and let $p(x)=\E u\,\big[p(u)\wedge\phi(u,x)\big]$. By analyticy $p(\U)=\phi(s,\U)$.
\end{proof}


\rule{\textwidth}{.6pt}\\[1ex]
