...
The elements of $S_n$ are called \emph{standard samples\/} (we will mainly use $\NN$-valued samples and interpret these as finite multisets i.e.\@ sets where elements can occur with different multiplicity).
...
It contains $L$ and a symbol for every subset of $\RR^n$ and for every function $\RR^n\to\RR$. 
...
The first one is denoted by \emph{$\ns\bar\U=\<\ns\U,\ns\RR,(\nsS_n)_{n\in\omega}\>$.}
This is an elementary extension of $\bar\U$ that is saturated and has cardinality $>\kappa$.
The second extension is an intermediate saturated model \emph{$\nso\U$} such that $\bar\U\preceq\nso\bar\U\preceq\ns\bar\U$ and that $|\nso\U|=\kappa$.
As $\kappa$ is inaccessible such model exist and we can assume that the home sort $\nso\bar\U$ is $\U$.
The other sorts are donoted by $\nso\RR$ and $(\nso S_n)_{n\in\omega}$.
...
\begin{remark}
  ...
  Intuitively the samples in $\ns\bar\U$ corresponds to the global Keisler measures while the samples in $\nso\bar\U$ corresponds to the smoohth measures.\QED
\end{remark}
...
In other words, $s$ is pseudofinite if $\bar\tp(s/\bar M)$ is finitely satisfied in $\bar M$ (we will not further mention finite satisfiability in this context to avoid clash with the terminology in the following sections). By Fact~\ref{fact_elementarity}, every sample is pseudofinite over any $\omega$-saturated model.
...
If for a given $\phi(x,z)\in L$ and $\epsilon\in\RR^+$, finitely many pairs of formulas $\psi_1(x),\psi_2(x)$ suffices for all $b\in\U^{|z|}$, we say that $s$ is \emph{uniformly smooth.} 
...
\begin{fact}
  ...
  \begin{itemize}
    ...
    \item[3.] if $t\in\nsS_{|x|}$ is non-negative and $\mu_s\psi(x)\approx\mu_t\psi(x)$ for all $\psi(x)\in L(A)$, then $\mu_s\approx\mu_t$.
  \end{itemize}
\end{fact}
