...
The elements of $S_n$ are called \emph{standard samples\/} (we will mainly use $\NN$-valued samples and interpret these as finite multisets).
...
The domains of the various sorts of $\ns\bar\U$ are denoted by \emph{$\<\ns\U,\ns\RR,(\nsS_n)_{n\in\omega}\>$.}
The second extension is an intermediate saturated model that we denote by $\nso\U$.
We require that $\bar\U\preceq\nso\bar\U\preceq\ns\bar\U$ and $|\nso\U|=\kappa$.
As $\kappa$ is inaccessible such model exist.
We can assume that the home sort $\nso\bar\U$ is $\U$.
The other sorts are donoted by $\nso\RR$ and $(\nso S_n)_{n\in\omega}$.

The model $\nso\bar\U$ plays the role of the moster model of $\bar T=\Th(\bar\U)$, while $\ns\bar\U$ is a model where all global types over $\nso\bar\U$ are realized.
...
\begin{fact}
  ...
  \begin{itemize}
    ...
    \item[3.] for every $t\in\nsS_{|x|}$, if $\mu_s\psi(x)\approx\mu_t\psi(x)$ for all $\psi(x)\in L(A)$, then $\mu_s\approx\mu_t$.
  \end{itemize}
\end{fact}
