...
\section{Loeb samples (bootstrapping)}

In this section we introduce hypefinite samples and prove (Lemma~\ref{lem_existence_sample}) that all Keisler measures are generated by some Loeb sample.
...
Let \emph{$\bar\U$\/} be the multi-sorted structure $\<\U,\RR,(S_n)_{n\in\omega}\>$.
Clearly, $|\bar\U|=\kappa$.
...
The larger one is denoted by \emph{$\ns\bar\U=\<\ns\U,\ns\RR,(\nsS_n)_{n\in\omega}\>$.}
This is an elementary extension of $\bar\U$ that is saturated and has cardinality $>\kappa$.
...
\begin{lemma}\label{lem_existence_sample}
  Let $\mu$ be finitely additive signed measures on a Boolean algebra $\Delta\subseteq L_x(\U)$.
  Then there is $s\in\nsS_{|x|}$ such that 
...
\end{corollary}
If $\mu_s\phi(x)\approx\mu_t\phi(x)$ for every $\phi(x)\in L(M)$ we write \emph{$\mu_s{\restriction} M=\mu_t{\restriction} M$.}

\begin{question}
  Are the following equivalent?
  \begin{itemize}
    \item[1.] $s\equiv_Mt$;
    \item[2.] $\mu_s{\restriction} M=\mu_t{\restriction} M$?
  \end{itemize}
\end{question}
...
It perfectly translate to samples.
These approach clarify the intuition behind: smooth samples are those that are equivalent to a sample in $\nso\bar\U$. 

We say that a non-negative sample $s\in\nsS_{|x|}$ is \emph{smooth\/} over $A$, if for every $\phi(x,z)\in L$, every $b\in\U^{|z|}$ and every $\epsilon\in\RR^+$ there are two formulas $\psi_1(x),\psi_2(x)\in L(M)$ such that $\psi_1(x)\imp\phi(x,b)\imp\psi_2(x)$ and 
...
If for a given $\phi(x,z)$ and $\epsilon$, finitely many pairs of formulas $\psi_1(x),\psi_2(x)$ suffices for all $b\in\U^{|z|}$, we say that $s$ is \emph{uniformly smooth.}
More precisely, $s$ is uniformly smooth if for every $\phi(x,z)\in L$ and every $\epsilon\in\RR^+$ there is an $n$ and some formulas $\psi_{1,i}(x),\psi_{2,i}(x)\in L(M)$, for $i=1,\dots,n$, such that 

\noindent\kern10ex$\displaystyle\A z\bigvee_{i=1}^n\big[\psi_{1,i}(x)\imp\phi(x,z)\imp\psi_{2,i}(x)\big]$

and

\ceq{\hfill\mu_s\psi_{1,i}(x)}{\approx_\epsilon}{\mu_s\psi_{2,i}(x)}\hfill for $i=1,\dots,n$.

\begin{fact}
  The following are equivalent for every non-negative sample $s\in\nsS_{|x|}$
  \begin{itemize}
    \item[1.] $s$ is smooth over $A$;
    \item[2.] $s$ is uniformly smooth over $A$; 
    \item[3.] if $t\in\nsS_{|x|}$ is non-negative and $\mu_s{\restriction} A\approx\mu_t{\restriction} A$, then $\mu_s\approx\mu_t$.
  \end{itemize}
\end{fact}
...
\begin{proof}
  ...
  Then $\mu_s{\restriction} A\approx\mu_t{\restriction} A$ but $\mu_s\phi(x)\napprox_\epsilon\mu_t\phi(x)$ for some $\epsilon\in\RR^+$ and some formula $\phi(x)\in L(\U)$.
  Suppose $s$ is smooth and let $\psi_1(x),\psi_2(x)\in L(A)$ as above but with $\epsilon/3$ for $\epsilon$.
