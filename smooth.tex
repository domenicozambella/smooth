...
\begin{example}
  This example shows that in the fact above can fail without the assumption of saturation.
  Let $\U=\ns\NN$ and $M=\NN$.
  Then $\bar\U=\<\ns\NN,\RR,(S_n)_{n\in\omega}\>$ and $\bar M=\<\NN,\RR,(S_n{\restriction}\NN)_{n\in\omega}\>$. 
  Let $x,y$ and $u$ be variables of sort $\U$ and $S_1$, respectively.
  Then $\bar\U\notmodels \A x\; \E u\; \A y<x\; (u\,y>0)$ but this sentence holds in $\bar M$.\QED
\end{example}
...
The following notion will be proven redundant in Fact~\ref{fact_smoothness} but it is introduced for emphasis.
...
\begin{proof}
  ...
  \ceq{\hfill\mu_t\phi(x,b)}{\approx}{\ \,\sup\big\{\mu_t\psi(x)\ :\ \psi(x)\imp\phi(x,b)\big\}.}
...
There are a few distinct notions of finite satisfiabilty that apply to this context.
Pseudo\-finiteness it is the notion that best fit with that of analytic samples.
(A weaker notion of finite satisfiability that corresponds to that of Keisler measures will be introduced later.)
